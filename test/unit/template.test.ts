...
describe('MarkupTemplate', () => {
    ...
    it('falls back to parent context inside array sections', () => {
        expect(
            MarkupTemplate.render(
                '{{#items}}{{prefix}}:{{name}};{{/items}}',
                {
                    prefix: 'item',
                    items: [
                        { name: 'A' },
    ...
    it('renders nested sections with the same name', () => {
        expect(
            MarkupTemplate.render(
                '{{#items}}[{{name}}{{#items}}:{{name}}{{/items}}]{{/items}}',
                {
                    items: [
                        {
                            name: 'A',
    ...
    it('throws on unclosed double brace tag', () => {
        expect(() =>
            MarkupTemplate.render('Hello {{name', {
                name: 'Alice'
            })
    ...
    it('throws on unexpected closing section', () => {
        expect(() =>
            MarkupTemplate.render('{{/items}}', {})
        ).toThrow('Unexpected closing template section: items')
    })
