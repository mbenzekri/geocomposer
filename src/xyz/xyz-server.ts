...
export function createXyzApp(options: XyzAppOptions): XyzApp {
  const path = normalizePath(options.path ?? '/tiles')
  const tileSize = options.tileSize ?? DEFAULT_TILE_SIZE
  const minZoom = options.minZoom ?? DEFAULT_MIN_ZOOM
  const maxZoom = options.maxZoom ?? DEFAULT_MAX_ZOOM
  const maxScaleFactor = options.maxScaleFactor ?? DEFAULT_MAX_SCALE_FACTOR
  const layerByName = new Map(options.layers.map((layer) => [layer.name, layer]))
  const sources = uniqueSources(options.layers)
  let nextTraceId = 1

  validateXyzOptions(tileSize, minZoom, maxZoom, maxScaleFactor)
  ...
  return {
    ...
    async handle(req, res) {
      ...
      try {
        ...
        if (!isPathMatch(url.pathname, path)) {
        ...
        const cachedImage = options.cache
          ? await readCachedTile(options.cache, tileRequest)
          : null
        ...
        if (!cachedImage && options.cache) {
          await writeCachedTile(options.cache, tileRequest, image)
        }
