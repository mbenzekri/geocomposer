...
export function featureInfoToXml(result: FeatureInfoResult): string {
  ...
  const layerXml = [...layers.entries()].map(([layerName, hits]) => [
    `<Layer name="${escapeXml(layerName)}">`,
  ...
  return [
    ...
    `<FeatureInfoResponse version="1.3.0" crs="${escapeXml(result.crs)}" numberReturned="${result.hits.length}">`,
...
function featureToXml(feature: Feature): string {
  const id = feature.id === undefined ? '' : ` id="${escapeXml(String(feature.id))}"`
  return [
    `<Feature${id}>`,
    propertiesToXml(feature.properties),
    feature.geometry ? `<Geometry type="${escapeXml(feature.geometry.type)}" encoding="GeoJSON">${escapeXml(JSON.stringify(feature.geometry))}</Geometry>` : '',
    '</Feature>'
  ].join('')
}
...
function propertiesToXml(properties: Props | null): string {
  ...
  const propertyXml = Object.entries(properties).map(([name, value]) => {
    if (value === null || value === undefined) {
      return `<Property name="${escapeXml(name)}" nil="true"/>`
    }

    return `<Property name="${escapeXml(name)}" type="${escapeXml(propertyType(value))}">${escapeXml(propertyValue(value))}</Property>`
  }).join('')
